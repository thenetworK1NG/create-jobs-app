...
// Function to save job card with assigned person
function saveJobCard(assignedTo) {
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      // Get the assigned person directly from the form
      const assignedPerson = document.getElementById('assignedPerson').value;
      if (!assignedPerson) {
        showStatus('Please select a team member to assign the job to.');
        document.getElementById('assignedPerson').style.border = '2px solid #b23c3c';
        return;
      }
      
      // Reset border color if valid
      document.getElementById('assignedPerson').style.border = '2px solid #e5e7eb';
      
      // Directly save the job card with the assigned person
      saveJobCard(assignedPerson);
    });
  }
